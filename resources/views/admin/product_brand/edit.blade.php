...
@section('title', 'Pricewise- Product Brand Edit')
@section('content')
        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><a
                            href="{{ route('admin.product-brands.index') }}">Brands</a>
                    </li>
                </ol>
            </nav>
        </div>
                                ...
                                <select class="form-control" name="category">
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}"
                                            {{ $brand->category == $category->id ? 'selected' : '' }}>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="">
                                <select name="status" id="status" class="form-control">
                                    <option value="active" {{ $brand->status == 'active' ? 'selected' : '' }}>Yes</option>
                                    <option value="inactive" {{ $brand->status == 'inactive' ? 'selected' : '' }}>No
                                    </option>
                                </select>
                            </div>
                        </div>
