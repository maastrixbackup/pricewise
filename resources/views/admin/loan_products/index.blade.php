...
@section('title', 'PriceWise- Loan Products')
@section('content')
    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        ...
        {{-- <div class="row">
            <select name="" id="purpose" class="form-control" onchange="showData()">
                <option value="">Select</option>
                @foreach (App\Models\SpendingPurpose::all() as $v)
        ...
        <div class="ms-auto">
            ...
            <div class="btn-group">
                <a href="{{ route('admin.loans.create') }}" class="btn btn-primary">Create Loan Produt</a>
                                ...
                                @if ($loans)
                                    @foreach ($loans as $val)
                                        <tr>
                                            ...
                                            <td>{{ $val->title }}</td>
                                            <td>{{ $val->borrow_amount }}</td>
                                            <td>{{ $val->purposeDetails->name }}</td>
                                            {{-- <td>
                                                @if (!empty($val->image))
                                                    <img src="{{ asset('storage/images/loans/' . $val->image) }}"
                                                        alt="{{ $val->image }}" width="100px" height="60px">
                                                @else
                                            ...
                                            <td>
                                                <div class="col">
                                                    <a title="Edit" href="{{ route('admin.loans.edit', $val->id) }}"
                                                    ...
                                                    <a title="Delete" class="btn1 btn-outline-danger trash remove-category"
                                                        data-id="{{ $val->id }}"
                                                        data-action="{{ route('admin.loans.destroy', $val->id) }}"><i
