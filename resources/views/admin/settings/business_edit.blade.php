        ...
        <div class="card">
            ...
            <div class="card-body p-4">
                <form id="mailSetForm" method="post" action="{{route('admin.business-store')}}">
                    @csrf
                    @foreach($businessSetting as $key=>$business2)
                    <div class="card-header px-4 py-3">
                        <h4 class="mb-0" style="margin-left: -20px !important">{{strtoupper($key)}}</h4>
                    </div>
                    @foreach($business2 as $business)
                    <div class="mb-3">
                        <label for="host" class="col-form-label">{{strtoupper(str_replace('_', ' ', $business->key))}}</label>
                        <input class="form-control" type="text" name="business[{{$business->key}}]" value="{{$business->value}}">
                    </div>
                     @endforeach
                    @endforeach
