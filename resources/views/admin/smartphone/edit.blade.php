                        ...
                        <div class="row mb-3">
                            ...
                            <div class="">
                                <input type="text" class="form-control" name="provider_name" id="provider_name"
                                    value="{{ $sp_record->provider_name }}" placeholder="Provider Name">
                                @error('provider_name')
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="">
                                <input type="text" class="form-control" name="provider_url" id="provider_url"
                                    value="{{ $sp_record->slug }}" placeholder="Provider Url" readonly>
                                @error('provider_url')
                                    <div class="alert alert-danger mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <select name="category_id" class="form-control" id="category_id">
                                    ...
                                    @if ($categories)
                                        @foreach ($categories as $item => $val)
                                            <option value="{{ $val->id }}" {{$sp_record->category_id == $val->id ? 'selected' : ''}}>{{ $val->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                                @error('category_id')
