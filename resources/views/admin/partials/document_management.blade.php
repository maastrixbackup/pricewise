<script>
    ...
    function createRow(documentName) {
        ...
        newRow.innerHTML = `
                ...
                <td>
                    ...
                    <button type="button" title="Upload" class="btn btn-primary upload-button"><i class="fa fa-upload" aria-hidden="true"></i></i></button>
                    <button title="Delete" class="btn btn-danger  delete-button"><i class="fa fa-trash" aria-hidden="true"></i></button>
                </td>
            `;
    ...
    addMoreButton.addEventListener("click", function() {
        const documentName =
            "New Document"; // You can set the default document name here or fetch it from user input
        ...
        if (target.classList.contains("upload-button")) {
            ...
            if (file) {
                ...
                xhr.setRequestHeader('X-CSRF-TOKEN',
                    '{{ csrf_token() }}'); // Include CSRF token if using Laravel CSRF protection
                xhr.onload = function() {
                    if (xhr.status === 200) {
