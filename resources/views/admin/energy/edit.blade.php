            ...
            <div class="tab-pane fade" id="meta" role="tabpanel">                               
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-7 col-12">
                            <div class="card-body p-4">
                                <input type="hidden" name="category_id" value="{{$objEnergy->category}}">
                                <input type="hidden" name="post_id" value="{{$objEnergy->id}}">
                                <table class="table">
                                    <thead>
                                        <tr>
                                            <th scope="col">#</th>
        ...
        documentList.addEventListener("click", function(event) {
            ...
            if (target.classList.contains("upload-button")) {
                ...
                if (file) {
                    ...
                    formData.append('category_id', "{{ $objEnergy->category }}");
                    formData.append('post_id', "{{ $objEnergy->id }}");
                    ...
                    xhr.open('POST', "{{route('admin.doc_update', $objEnergy->id)}}");
        ...
        documentList.addEventListener("click", (event) => {
            if (event.target.classList.contains("delete-button")) {
                ...
                const formData = new FormData();
                    ...
                    const xhr = new XMLHttpRequest();
                    xhr.open('POST', "{{route('admin.doc_delete', $objEnergy->id)}}");
                    xhr.setRequestHeader('X-CSRF-TOKEN', '{{ csrf_token() }}'); // Include CSRF token if using Laravel CSRF protection
                    xhr.onload = function() {
                        if (xhr.status === 200) {
