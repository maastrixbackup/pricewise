...
use App\Http\Resources\FarmHouseInsuranceResource;
...
class FarmHouseInsuranceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $products = InsuranceProduct::where('sub_category',  config('constant.subcategory.BuildingInsurance'))->with('postFeatures', 'categoryDetail', 'coverages.coverageDetails', 'providerDetails');
        ...
        $objFeatures = Feature::select('f1.id', 'f1.features', 'f1.input_type', DB::raw('COALESCE(f2.features, "No_Parent") as parent'))
            ...
            ->where(['f1.category' => config('constant.category.Insurance'), 'f1.sub_category' => config('constant.subcategory.BuildingInsurance')])
        ...
        foreach ($filteredProducts as $product) {
            $formattedProduct = (new FarmHouseInsuranceResource($product))->toArray($request);
        ...
        $coverages = insuranceCoverage::where('subcategory_id', config('constant.subcategory.BuildingInsurance'))->get();
    ...
    public function buildingInsuranceCompare(Request $request)
    {
        ...
        if (!empty($compareIds)) {
            ...
            if ($filteredProducts->isNotEmpty()) {
                ...
                $filteredProductsFormatted = FarmHouseInsuranceResource::collection($filteredProducts);
