...
class CatererController extends Controller
{
    ...
    public function postcaterer(Request $request){
        ...
        //     'caterer_name' => 'required|string|max:255',
        // ]);
       ...
       DB::table('caterers')
       ->insert(['caterer_name'=>$request->get('name'),'isenable'=>$request->get('isenable'),'created_at'=>NOW(),'updated_at'=>NOW()]);
    ...
    public function updatecaterer(Request $request,$id){
        ...
        DB::table('caterers')
        ->where('id',$id)
        ->update(['caterer_name'=>$request->get('name'),'isenable'=>$request->get('isenable'),'updated_at'=>NOW()]);
